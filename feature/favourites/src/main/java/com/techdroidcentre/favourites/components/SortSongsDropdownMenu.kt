package com.techdroidcentre.favourites.components
...
fun SortSongsDropdownMenu(
    ...
    val menuItems = FavouriteSongsSortOption.values().map {
        ...
        menuItems.forEach { item ->
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                DropdownMenuItem(
                    text = { Text(text = item) },
                    onClick = {
                        onSortSongs(FavouriteSongsSortOption.valueOf(item.uppercase()))
                    }
                )
            }
        }
